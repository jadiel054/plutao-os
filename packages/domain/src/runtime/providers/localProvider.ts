...
export class LocalProvider {
  ...
  /** Referência ao módulo Transformers.js importado */
  ...
  private statusListeners: Array<(status: LocalModelStatus) => void> = [];
  ...
  /**
   * Registra um listener de mudança de status
   */
  onStatusChange(listener: (status: LocalModelStatus) => void): () => void {
  ...
  private setStatus(newStatus: LocalModelStatus) {
  ...
  private async loadTransformers(): Promise<void> {
    ...
    this.loadPromise = (async () => {
      try {
        ...
        // Importa Transformers.js dinamicamente e guarda módulo
  ...
  private async loadPipeline(): Promise<void> {
    ...
    this.loadPromise = (async () => {
      try {
        ...
        const pipelineFn = this.transformersModule?.pipeline || (window as any).pipeline;
        ...
        this.pipeline = await pipelineFn("text-generation", this.modelId, {
  ...
  private parseToolProposal(content: string): { name: string; input: string } | null {
    ...
    try {
      ...
      const input = typeof parsed.input === "object" ? JSON.stringify(parsed.input) : String(parsed.input ?? "");
