...
import { WriteGateCard } from "@/components/WriteGateCard";
...
function parseGatePendingFromOutput(output: string | undefined | null): PendingWriteGate | null {
  if (!output || !output.includes("GATE_PENDING")) return null;
  const idMatch = output.match(
    /gate_id:\s*([0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12})/i
  );
  if (!idMatch?.[1]) return null;
  const capability = output.match(/capability:\s*(.+)/i)?.[1]?.trim() ?? "write";
  const target = output.match(/target:\s*(.+)/i)?.[1]?.trim() ?? "";
  const summary =
    output.match(/summary:\s*(.+)/i)?.[1]?.trim() ?? "Escrita pendente de aprovação";
  return {
    id: idMatch[1],
    provider: "github",
    capability,
...
function ChatPageInner() {
  ...
  const init = useCallback(async () => {
    ...
    try {
      ...
      try {
        const gRes = await fetch("/api/gates", { cache: "no-store" });
        if (gRes.ok) {
          const gData = await gRes.json();
          if (Array.isArray(gData.gates)) {
            setPendingGates(
              gData.gates
                .map((g: Record<string, unknown>) => ({
                  ...
                  contentPreview: g.contentPreview ? String(g.contentPreview) : null,
                  status: String(g.status ?? "pending"),
                  missionId: g.missionId ? String(g.missionId) : null,
                  createdAt: g.createdAt ? String(g.createdAt) : undefined,
                }))
                .filter((g: PendingWriteGate) => g.id.length > 0)
            );
          }
        }
      } catch { /* ignore */ }
          ...
          for (const part of parts) {
            ...
            try {
              ...
              if (eventName === "reasoning_step") {
                ...
                const outText =
                  (typeof parsed.fullOutput === "string" ? parsed.fullOutput : "") ||
                  (typeof parsed.summaryOutput === "string" ? parsed.summaryOutput : "");
                const gate = parseGatePendingFromOutput(outText);
                if (gate) {
                  setPendingGates((prev) =>
                    prev.some((g) => g.id === gate.id) ? prev : [...prev, gate]
                  );
                }
        ...
        {messages.length === 0 ? (
          ...
          <div className="flex-1 overflow-y-auto space-y-4 pb-4">
            ...
            {pendingGates.length > 0
              ? pendingGates.map((g) => (
                  <div key={g.id} className="flex justify-start">
                    <WriteGateCard
                      gateId={g.id}
                      provider={g.provider}
                      ...
                      onResolved={(result) => {
                        setPendingGates((prev) => prev.filter((x) => x.id !== g.id));
                        if (result.status === "executed") {
                          addToast("Escrita aprovada e executada.", "success");
                        } else if (result.status === "rejected") {
                          addToast("Escrita rejeitada. Nada foi alterado no provedor.", "info");
